...
def read_volunteers(committee_df, emergency_pool_volunteers_file_path):
    ...
    with open(emergency_pool_volunteers_file_path, encoding="utf-8") as f:
        ...
        for row in reader:
            ...
            if (
                full_name in all_full_names_with_count
                and all_full_names_with_count[full_name] > 1
            ):
                warnings.warn(
                    f"There are more than one {full_name} in the committee, this "
...
if __name__ == "__main__":
    ...
    bid_profile = committee_to_bid_profile(
        available_reviewers, submissions, bid_level_weights
    )
